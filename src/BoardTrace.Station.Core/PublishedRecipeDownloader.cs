...
{
    public async Task<LoadedClassicalRecipe> DownloadAsync(Guid versionId, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var assetId in version.Bundle.References.Select(reference => reference.AssetId).Distinct())
