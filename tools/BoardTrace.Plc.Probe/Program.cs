...
internal static class Program
{
    ...
    private static readonly CurrentUser Actor = new("fixture-operator", "operator", "Isolated PLC Operator", ["Operator"], null);
    private const string Scope = "隔离本地批准批次夹具，非质量发布；真实 Python TCP/Runner/Core/OpenCV/SQLite。未读 truth/test，未连接中央。";
    public static async Task Main(string[] args)
    {
        ...
        coordinator.StartBatch(new(Guid.NewGuid(), batch.Id, batch.StationId, batch.RecipeBundleHash, first.Id,
            Actor.Id, Actor.DisplayName, DateTimeOffset.UtcNow.AddMinutes(-1), DateTimeOffset.UtcNow.AddHours(1)));
