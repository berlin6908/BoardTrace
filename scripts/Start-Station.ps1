param(
    ...
    [string]$Credentials = ".local/stations/$Station.json"
)
...
try {
    ...
    & "$boardtraceRoot/src/BoardTrace.Station/bin/Release/net10.0-windows/BoardTrace.Station.exe" --station $Station --manifest $Manifest --database $Database --data-root "$boardtraceRoot/data" --server $Server --credentials $Credentials
} finally {
    Pop-Location
}
