    ...
    private sealed class BatchServer : IAsyncDisposable
    {
        ...
        public static async Task<BatchServer> CreateAsync(TimeSpan? cookieLifetime = null)
        {
            ...
            try
            {
                ...
                var bundle = new PublishedRecipeBundle(versionId, draftId, runId, "ISOLATED BATCH FIXTURE", new ClassicalRecipeDefinition(settings), targets, targets,
                    new(640, 640, true), new string('a', 64), new string('b', 64), new string('c', 64),
                    [new("sample", assetId, Convert.ToHexStringLower(SHA256.HashData(new byte[] { 1, 2, 3 })), 3)], null, "engineer", "engineer", DateTimeOffset.UtcNow);
                server.Version = new(bundle, PublishedRecipeTransfer.Hash(bundle));
                db.RecipeDrafts.Add(new RecipeDraft { Id = draftId, Name = bundle.Name, DefinitionJson = JsonSerializer.Serialize<RecipeDefinition>(new ClassicalRecipeDefinition(settings)), TargetsJson = JsonSerializer.Serialize(targets), DataManifestSha256 = bundle.InputManifestSha256, SnapshotHash = bundle.ValidationSnapshotHash, AuthorId = "engineer", UpdatedAt = DateTimeOffset.UtcNow });
                db.ValidationRuns.Add(new ValidationRun { Id = runId, DraftId = draftId, Name = bundle.Name, Status = "Completed", DefinitionJson = JsonSerializer.Serialize<RecipeDefinition>(new ClassicalRecipeDefinition(settings)), TargetsJson = JsonSerializer.Serialize(targets), ManifestHash = bundle.InputManifestSha256, TruthHash = new string('d', 64), SnapshotHash = bundle.ValidationSnapshotHash, CreatedAt = DateTimeOffset.UtcNow });
                db.RecipeVersions.Add(new RecipePublication { Id = versionId, DraftId = draftId, ValidationRunId = runId, Name = bundle.Name, BundleJson = JsonSerializer.Serialize(bundle, Json), BundleHash = server.Version.BundleHash, PublishedById = "engineer", PublishedByName = "engineer", PublishedAt = bundle.PublishedAt, Assets = [new RecipeAsset { Id = assetId, RecipeVersionId = versionId, Sha256 = bundle.References[0].Sha256, Content = [1, 2, 3] }] });
